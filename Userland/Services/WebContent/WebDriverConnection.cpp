...
public:
    explicit NavigationTimer(Web::DOM::Document& document)
        : m_document(document)
    ...
    void start(u64 timeout_ms, JS::NonnullGCPtr<JS::HeapFunction<void(Web::WebDriver::Response)>> on_timeout)
    {
        ...
        m_timer->on_timeout = [this]() {
            ...
            if (m_on_timeout) {
                if (m_document->page().has_pending_dialog())
                    m_on_timeout->function()(JsonValue {});
                else
...
private:
    ...
    JS::NonnullGCPtr<Web::DOM::Document> m_document;
...
void WebDriverConnection::wait_for_navigation_to_complete(OnNavigationComplete on_complete)
{
    ...
    auto timer = realm.heap().allocate<NavigationTimer>(realm, *current_browsing_context().active_document());
    timer->start(m_timeouts_configuration.page_load_timeout.value_or(300'000), on_complete);
