...
TEST_CASE(empty_string)
{
    ...
    (void)string.utf16_view(); // Convert to UTF-16 storage.
    ...
    EXPECT_EQ(string.length_in_code_units(), 0uz);
    EXPECT_EQ(string.length_in_code_points(), 0uz);
    EXPECT_EQ(string.utf16_view(), Utf16View {});
}
...
{
    {
        ...
        EXPECT_EQ(string.length_in_code_units(), 6uz);
        EXPECT_EQ(string.length_in_code_points(), 6uz);
        EXPECT_EQ(string.utf16_view(), Utf16View { u"hello!" });
    }
    {
        ...
        EXPECT_EQ(string.length_in_code_units(), 12uz);
        EXPECT_EQ(string.length_in_code_points(), 12uz);
        EXPECT_EQ(string.utf16_view(), Utf16View { u"hello there!" });
    }
    {
        ...
        EXPECT_EQ(string.utf16_view(), Utf16View { u"😀" });
    }
    {
        auto string = Utf16String::from_utf8("hello 😀 there!"sv);
        EXPECT(!string.is_empty());
    ...
    }
    {
        ...
        EXPECT_EQ(string.utf16_view(), Utf16View { u"hello \xd800!" });
    }
}
...
{
    {
        auto string = Utf16String::from_utf16(Utf16View { u"hello!" });
        EXPECT(!string.is_empty());
        EXPECT(string.is_ascii());
        EXPECT(!string.has_long_ascii_storage());
        EXPECT(string.has_short_ascii_storage());
    ...
    {
        auto string = Utf16String::from_utf16(Utf16View { u"hello there!" });
        EXPECT(!string.is_empty());
        EXPECT(string.is_ascii());
        EXPECT(string.has_long_ascii_storage());
        EXPECT(!string.has_short_ascii_storage());
    ...
    {
        auto string = Utf16String::from_utf16(Utf16View { u"😀" });
        EXPECT(!string.is_empty());
        EXPECT(!string.is_ascii());
        EXPECT(!string.has_long_ascii_storage());
        EXPECT(!string.has_short_ascii_storage());
...
TEST_CASE(equals)
{
    auto test = [](Utf16String const& string1, Utf16String const& string3) {
        ...
        EXPECT_NE(string2, string3);
        EXPECT_NE(string3, string1);
        EXPECT_NE(string3, string2);
    };
    ...
    // ASCII stored as UTF-16 comparison.
    auto ascii_string_stored_as_utf16 = "hello"_utf16;
    (void)ascii_string_stored_as_utf16.utf16_view();

    test(ascii_string_stored_as_utf16, "there"_utf16);
    test(ascii_string_stored_as_utf16, "hello there general!"_utf16);
    test(ascii_string_stored_as_utf16, "😀"_utf16);
}
...
TEST_CASE(equals_ignoring_ascii_case)
{
    auto test = [](Utf16String const& string1, Utf16String const& string3) {
        ...
        EXPECT(!string1.equals_ignoring_ascii_case(string3));
        EXPECT(!string2.equals_ignoring_ascii_case(string3));
        EXPECT(!string3.equals_ignoring_ascii_case(string1));
        EXPECT(!string3.equals_ignoring_ascii_case(string2));
    };
...
TEST_CASE(code_unit_at)
{
    auto test = [](Utf16View const& view, size_t length_in_code_units) {
        ...
        EXPECT_DEATH("Accessing a code unit past the end", (void)string.code_unit_at(length_in_code_units));
    };

    test({}, 0);
    test(u"hello", 5);
    test(u"hello there general!", 20);
    test(u"😀", 2);
    test(u"hello 😀 there!", 15);
}
...
TEST_CASE(code_point_at)
{
    auto test = [](Utf16View const& view, size_t length_in_code_points) {
        ...
        EXPECT_DEATH("Accessing a code point past the end", (void)string.code_point_at(string.length_in_code_units()));
    };
