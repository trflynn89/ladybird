...
void ViewImplementation::did_update_navigation_buttons_state(Badge<WebContentClient>, bool can_navigate_back, bool can_navigate_forward) const
{
    m_navigate_back_action->set_enabled(can_navigate_back ? Action::Enabled::Yes : Action::Enabled::No);
    m_navigate_forward_action->set_enabled(can_navigate_forward ? Action::Enabled::Yes : Action::Enabled::No);
}
...
void ViewImplementation::initialize_context_menus()
{
    ...
    m_navigate_back_action = Action::create("Go Back"_string, ActionID::NavigateBack, [this]() {
        traverse_the_history_by_delta(-1);
    });
    m_navigate_forward_action = Action::create("Go Forward"_string, ActionID::NavigateForward, [this]() {
        traverse_the_history_by_delta(+1);
    });
    m_reload_action = Action::create("&Reload"_string, ActionID::Reload, [this]() {
        reload();
    });

    m_navigate_back_action->set_enabled(Action::Enabled::No);
    m_navigate_forward_action->set_enabled(Action::Enabled::No);

    m_search_selected_text_action = Action::create("&Search Selected Text"_string, ActionID::SearchSelectedText, [this]() {
    ...
    m_search_selected_text_action->set_visible(Action::Visible::No);
    ...
    m_take_visible_screenshot_action = Action::create("Take &Visible Screenshot"_string, ActionID::TakeVisibleScreenshot, [take_and_save_screenshot]() {
        take_and_save_screenshot(ScreenshotType::Visible);
    });
    m_take_full_screenshot_action = Action::create("Take &Full Screenshot"_string, ActionID::TakeFullScreenshot, [take_and_save_screenshot]() {
        take_and_save_screenshot(ScreenshotType::Full);
    });

    m_open_in_new_tab_action = Action::create("Open in New &Tab"_string, ActionID::OpenInNewTab, [this]() {
        if (on_link_click)
            on_link_click(m_context_menu_url, {}, Web::UIEvents::Mod_Ctrl);
    });
    m_copy_url_action = Action::create("Copy &URL"_string, ActionID::CopyURL, [this]() {
        insert_text_into_clipboard(url_text_to_copy(m_context_menu_url));
    });

    m_open_image_action = Action::create("&Open Image"_string, ActionID::OpenImage, [this]() {
        if (on_link_click)
            on_link_click(m_context_menu_url, {}, {});
    });
    m_copy_image_action = Action::create("Copy &Image"_string, ActionID::CopyImage, [this]() {
    ...
    m_open_audio_action = Action::create("&Open Audio"_string, ActionID::OpenAudio, [this]() {
        if (on_link_click)
            on_link_click(m_context_menu_url, {}, {});
    });
    m_open_video_action = Action::create("&Open Video"_string, ActionID::OpenVideo, [this]() {
        if (on_link_click)
            on_link_click(m_context_menu_url, {}, {});
    });
    m_media_play_action = Action::create("&Play"_string, ActionID::PlayMedia, [this]() {
        client().async_toggle_media_play_state(page_id());
    });
    m_media_pause_action = Action::create("&Pause"_string, ActionID::PauseMedia, [this]() {
        client().async_toggle_media_play_state(page_id());
    });
    m_media_mute_action = Action::create("&Mute"_string, ActionID::MuteMedia, [this]() {
        client().async_toggle_media_mute_state(page_id());
    });
    m_media_unmute_action = Action::create("Un&mute"_string, ActionID::UnmuteMedia, [this]() {
        client().async_toggle_media_mute_state(page_id());
    });
    m_media_controls_action = Action::create_checkable("&Show Controls"_string, ActionID::ToggleMediaControlsState, [this]() {
        client().async_toggle_media_controls_state(page_id());
    });
    m_media_loop_action = Action::create_checkable("&Loop"_string, ActionID::ToggleMediaLoopState, [this]() {
        client().async_toggle_media_loop_state(page_id());
    });

    m_page_context_menu = adopt_ref(*new Menu("Page Context Menu"sv));
    ...
    m_link_context_menu = adopt_ref(*new Menu("Link Context Menu"sv));
    m_link_context_menu->add_action(*m_open_in_new_tab_action);
    m_link_context_menu->add_action(*m_copy_url_action);

    m_image_context_menu = adopt_ref(*new Menu("Image Context Menu"sv));
    ...
    m_media_context_menu = adopt_ref(*new Menu("Media Context Menu"sv));
...
void ViewImplementation::did_request_page_context_menu(Badge<WebContentClient>, Gfx::IntPoint content_position)
{
    ...
    if (m_search_text.has_value()) {
        ...
        m_search_selected_text_action->set_visible(Action::Visible::Yes);
    } else {
        m_search_selected_text_action->set_visible(Action::Visible::No);
    }
...
void ViewImplementation::did_request_link_context_menu(Badge<WebContentClient>, Gfx::IntPoint content_position, URL::URL url)
{
    ...
    m_open_in_new_tab_action->set_text("Open in New &Tab"_string);

    switch (url_type(m_context_menu_url)) {
    case URLType::Email:
        m_copy_url_action->set_text("&Copy Email Address"_string);
        break;
    case URLType::Telephone:
        m_copy_url_action->set_text("&Copy Phone Number"_string);
        break;
    case URLType::Other:
        m_copy_url_action->set_text("&Copy Link Address"_string);
        break;
    }
...
void ViewImplementation::did_request_image_context_menu(Badge<WebContentClient>, Gfx::IntPoint content_position, URL::URL url, Optional<Gfx::ShareableBitmap> bitmap)
{
    ...
    m_open_in_new_tab_action->set_text("Open Image in New &Tab"_string);
    m_copy_url_action->set_text("&Copy Image URL"_string);

    m_copy_image_action->set_enabled(m_image_context_menu_bitmap.has_value() ? Action::Enabled::Yes : Action::Enabled::No);
...
void ViewImplementation::did_request_media_context_menu(Badge<WebContentClient>, Gfx::IntPoint content_position, Web::Page::MediaContextMenu menu)
{
    ...
    auto name = menu.is_video ? "Video"sv : "Audio"sv;
    m_open_in_new_tab_action->set_text(MUST(String::formatted("Open {} in New &Tab", name)));
    m_copy_url_action->set_text(MUST(String::formatted("&Copy {} URL", name)));

    m_open_audio_action->set_visible(menu.is_video ? Action::Visible::No : Action::Visible::Yes);
    m_open_video_action->set_visible(menu.is_video ? Action::Visible::Yes : Action::Visible::No);

    m_media_play_action->set_visible(menu.is_playing ? Action::Visible::No : Action::Visible::Yes);
    m_media_pause_action->set_visible(menu.is_playing ? Action::Visible::Yes : Action::Visible::No);

    m_media_mute_action->set_visible(menu.is_muted ? Action::Visible::No : Action::Visible::Yes);
    m_media_unmute_action->set_visible(menu.is_muted ? Action::Visible::Yes : Action::Visible::No);

    m_media_controls_action->set_checked(menu.has_user_agent_controls ? Action::Checked::Yes : Action::Checked::No);
    m_media_loop_action->set_checked(menu.is_looping ? Action::Checked::Yes : Action::Checked::No);
