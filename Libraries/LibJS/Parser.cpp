...
NonnullRefPtr<RegExpLiteral const> Parser::parse_regexp_literal()
{
    ...
    auto pattern = Utf16String::from_utf16_without_validation(pattern_view.substring_view(1, pattern_view.length_in_code_units() - 2));
...
RefPtr<BindingPattern const> Parser::synthesize_binding_pattern(Expression const& expression)
{
    ...
    Lexer lexer { Utf16String::from_utf16_without_validation(source), m_state.lexer.filename(), expression.source_range().start.line, expression.source_range().start.column };
    Parser parser { lexer };
        ...
        if (is_object) {
            ...
            if (allow_member_expressions == AllowMemberExpressions::Yes && is_rest) {
                ...
                if (match(TokenType::StringLiteral)) {
                    ...
                    name = create_identifier_and_register_in_current_scope({ m_source_code, rule_start.position(), position() }, Utf16FlyString::from_utf16_without_validation(string_value.view().substring_view(0, string_value.length_in_code_units() - 1)));
                } else {
