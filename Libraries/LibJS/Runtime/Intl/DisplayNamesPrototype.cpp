...
JS_DEFINE_NATIVE_FUNCTION(DisplayNamesPrototype::of)
{
    ...
    code = TRY(canonical_code_for_display_names(vm, display_names->type(), code.as_string().string().to_utf8_but_should_be_ported_to_utf16()));
    auto code_string = code.as_string().string().to_utf8_but_should_be_ported_to_utf16();
