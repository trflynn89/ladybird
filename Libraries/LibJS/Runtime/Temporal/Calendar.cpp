...
static constexpr auto HEBREW_ADAR_I_MONTH_CODES = to_array({ Unicode::HEBREW_ADAR_I_MONTH_CODE });
...
static constexpr auto CHINESE_AND_DANGI_ISO_REFERENCE_YEARS = to_array<ISOReferenceYears>({
    ...
    { "M10L"sv, 1984, {}   },
    { "M11"sv,  1972, 1970 },
    { "M11L"sv, 0,    {}   }, // The reference year for days 1-29 differ and is handled below.
    { "M12"sv,  1972, 1972 },
    { "M12L"sv, {},   {}   },
    // clang-format on
});

[[maybe_unused]] static Optional<i32> chinese_or_dangi_reference_year(String const& calendar, StringView month_code, u8 day)
{
...
Vector<String> const& available_calendars()
{
    ...
    static auto calendars = []() {
        ...
        //
        //     In the meantime, we don't include them here as they do not appear in the list of supported calendars that
        //     test262 relies on. See:
...
ThrowCompletionOr<ISODate> non_iso_calendar_date_to_iso(VM& vm, String const& calendar, CalendarFields const& fields, Overflow overflow)
{
    ...
    if (fields.month_code.has_value()) {
        ...
        TRY(constrain_month_code(vm, calendar, *fields.year, *fields.month_code, overflow));
    }
    ...
    auto days_in_month = calendar_days_in_month(calendar, *fields.year, static_cast<u8>(*fields.month));
...
//
// The implementation-defined abstract operation NonISOMonthDayToISOReferenceDate takes arguments calendar (a calendar type
// that is not "iso8601"), fields (a Calendar Fields Record), and overflow (constrain or reject) and returns either a normal
// completion containing an ISO Date Record or a throw completion. It performs implementation-defined processing to convert
// fields, which represents a calendar date without a year (i.e., month code and day pair, or equivalent) in the built-in
// calendar identified by calendar, to a corresponding reference date in the ISO 8601 calendar as described below, subject
// to overflow correction specified by overflow. For reject, values that do not form a valid date cause an exception to be
// thrown. For constrain, values that do not form a valid date are clamped to their respective valid range as in CalendarDateToISO.
//
// The fields of the returned ISO Date Record represent a reference date in the ISO 8601 calendar that, when converted to
// calendar, corresponds to the month code and day of fields in an arbitrary but deterministically chosen reference year.
// The reference date is the latest ISO 8601 date corresponding to the calendar date that is between January 1, 1900 and
// December 31, 1972 inclusive. If there is no such date, it is the earliest ISO 8601 date corresponding to the calendar
// date between January 1, 1973 and December 31, 2035 inclusive. The reference year is almost always 1972 (the first ISO 8601
// leap year after the epoch), with exceptions for calendars where some dates (e.g. leap days or days in leap months)
...
{
    return Unicode::calendar_date_from_iso(calendar, iso_date);
}
...
bool calendar_supports_era(String const& calendar)
{
    ...
    return Unicode::calendar_supports_era(calendar);
}
...
bool year_contains_month_code(String const& calendar, i32 arithmetic_year, StringView month_code)
{
    ...
    if (calendar.is_one_of("chinese"sv, "dangi"sv)) {
        ...
        for (u8 month = 1; month <= months_in_year; ++month) {
            auto info = Unicode::chinese_ordinal_month_info(calendar, arithmetic_year, month);
            if (info.has_value() && info->is_leap && info->month_number == month_number)
                return true;
        }
...
ThrowCompletionOr<StringView> constrain_month_code(VM& vm, String const& calendar, i32 arithmetic_year, StringView month_code, Overflow overflow)
{
    // 1. Assert: IsValidMonthCodeForCalendar(calendar, monthCode) is true.
    ASSERT(is_valid_month_code_for_calendar(calendar, month_code));
    ...
    VERIFY(month_code == Unicode::HEBREW_ADAR_I_MONTH_CODE);
...
u8 month_code_to_ordinal(String const& calendar, i32 arithmetic_year, StringView month_code)
{
    ...
    ASSERT(!row->additional_month_codes.contains_slow("M00L"sv));
    ASSERT(!row->additional_month_codes.contains_slow("M13"sv));
...
{
    auto result = Unicode::iso_date_from_calendar(calendar, arithmetic_year, ordinal_month, day);
    if (!result.has_value())
        return vm.throw_completion<RangeError>(ErrorType::TemporalInvalidISODate);

    return create_iso_date_record(result->year, result->month, result->day);
}
