...
u64 create_vary_key(HeaderList const& request_headers, HeaderList const& response_headers)
{
    ...
    response_headers.for_each_header_value("Vary"sv, [&](StringView header) {
        header = header.trim_whitespace();
...
{
    return cache_directory.append(MUST(String::formatted("{:016x}_{:016x}", cache_key, vary_key)));
}
...
bool is_cacheable(u32 status_code, HeaderList const& headers)
{
    ...
    // AD-HOC: If the response contains `Vary: *`, we don't cache it for now, as it "signals that other aspects of the
    ...
    headers.for_each_header_value("Vary"sv, [&](StringView header) {
        if (header.trim_whitespace() == "*"sv) {
...
ByteString normalize_request_vary_header_values(StringView header, HeaderList const& request_headers)
{
    ...
    if (header.is_one_of_ignoring_ascii_case("Accept"sv, "Accept-Encoding"sv, "Accept-Language"sv)) {
        ...
        request_headers.for_each_header_value(header, [&](ByteString const& value) {
            values.append(value.to_lowercase().trim_whitespace());
            return IterationDecision::Continue;
        });
