...
Optional<CacheIndex::Entry&> CacheIndex::find_entry(u64 cache_key, HeaderList const& request_headers)
{
    auto& entries = m_entries.ensure(cache_key, [&]() {
        Vector<Entry> entries;

        m_database->execute_statement(
            m_statements.select_entry,
            [&](auto statement_id) {
                ...
                entries.empend(move(url), deserialize_headers(request_headers), deserialize_headers(response_headers), data_size, request_time, response_time, last_access_time);
            },
            cache_key);
    ...
    for (auto& entry : entries) {
        if (is_vary_mismatch(request_headers, entry.request_headers, entry.response_headers))
            continue;

        if (!result.has_value() || is_higher_ranked_request(entry.request_headers, result->request_headers))
            result = entry;
    }
