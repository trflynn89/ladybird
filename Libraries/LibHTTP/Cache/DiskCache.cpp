...
ErrorOr<bool> DiskCache::store_associated_data(URL::URL const& url, StringView method, HeaderList const& request_headers, u64 vary_key, CacheEntryAssociatedData associated_data, ReadonlyBytes data)
{
    if (!is_cacheable(method, request_headers))
        return false;
