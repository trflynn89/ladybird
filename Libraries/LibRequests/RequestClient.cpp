...
RefPtr<Request> RequestClient::start_request(ByteString const& method, URL::URL const& url, HTTP::ReloadRequest reload_request, Optional<HTTP::HeaderList const&> request_headers, ReadonlyBytes request_body, Core::ProxyData const& proxy_data)
{
    auto request_id = m_next_request_id++;
    auto headers = request_headers.map([](auto const& headers) { return headers.headers().span(); }).value_or({});

    IPCProxy::async_start_request(request_id, method, url, reload_request, headers, request_body, proxy_data);
