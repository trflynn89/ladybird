...
{
    StringBuilder builder(StringBuilder::Mode::UTF16, length_in_code_units() * 2);
...
bool Utf16View::is_ascii() const
{
    if (has_ascii_storage())
        return true;
    return simdutf::validate_ascii(reinterpret_cast<char const*>(m_string.utf16), length_in_code_units() * sizeof(char16_t));
}
