...
NonnullRefPtr<Utf16StringData> Utf16StringData::from_utf8(StringView utf8_string, AllowASCIIStorage allow_ascii_storage)
{
    ...
    VERIFY(utf8_string.length() >> Detail::Utf16StringData::UTF16_FLAG == 0);
    ...
    if (allow_ascii_storage == AllowASCIIStorage::Yes && utf8_string.is_ascii()) {
    ...
    } else if (Utf8View view { utf8_string }; view.validate(Utf8View::AllowSurrogates::No)) {
        ...
        for (auto code_point : view)
            code_unit_length += code_point < Utf16View::first_supplementary_plane_code_point ? 1 : 2;

        string = create_uninitialized(StorageType::UTF16, code_unit_length);

        for (auto code_point : view) {
            if (code_point < Utf16View::first_supplementary_plane_code_point) {
                string->m_utf16_data[code_unit_index++] = static_cast<u16>(code_point);
            } else {
...
{
    // Due to internal optimizations, we have an explicit maximum string length of 2**63 - 1.
    VERIFY(utf16_string.length_in_code_units() >> Detail::Utf16StringData::UTF16_FLAG == 0);

    RefPtr<Utf16StringData> string;

    if (utf16_string.is_ascii()) {
        string = create_uninitialized(StorageType::ASCII, utf16_string.length_in_code_units());

        auto result = simdutf::convert_utf16_to_utf8(utf16_string.char_data(), utf16_string.length_in_code_units(), string->m_ascii_data);
        ...
        TypedTransfer<char16_t>::copy(string->m_utf16_data, utf16_string.char_data(), utf16_string.length_in_code_units());
    }
