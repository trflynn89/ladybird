...
[[nodiscard]] ALWAYS_INLINE Utf16FlyString operator""_utf16_fly_string(char const* string, size_t length)
{
    StringView view { string, length };

    ASSERT(Utf8View { view }.validate());
    return Utf16FlyString::from_utf8_without_validation(view);
}

[[nodiscard]] ALWAYS_INLINE Utf16FlyString operator""_utf16_fly_string(char16_t const* string, size_t length)
{
    Utf16View view { { reinterpret_cast<u16 const*>(string), length } };

    ASSERT(view.validate());
    return Utf16FlyString::from_utf16_without_validation(view);
}
