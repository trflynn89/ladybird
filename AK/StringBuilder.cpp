...
static constexpr size_t string_builder_prefix_size(StringBuilder::Mode mode)
{
    ...
    case StringBuilder::Mode::UTF16:
        return sizeof(Detail::Utf16StringData);
    }
    VERIFY_NOT_REACHED();
}
...
StringBuilder::StringBuilder(Mode mode, size_t initial_capacity)
    : m_buffer(MUST(create_buffer(mode, initial_capacity)))
    , m_mode(mode)
{
}
...
Utf16View StringBuilder::utf16_string_view() const
{
    ...
    return Utf16View { { reinterpret_cast<u16 const*>(view.data()), view.size() / 2 } };
}
