...
- (void)createContextMenus
{
    ...
    self.page_context_menu = Ladybird::create_application_menu([self view].page_context_menu(), [weak_self](Gfx::IntPoint position) {
        if (LadybirdWebView* strong_self = weak_self) {
            auto* event = Ladybird::create_context_menu_mouse_event(strong_self, position);
            [NSMenu popUpContextMenu:strong_self.page_context_menu withEvent:event forView:strong_self];
        }
    });

    self.link_context_menu = Ladybird::create_application_menu([self view].link_context_menu(), [weak_self](Gfx::IntPoint position) {
        if (LadybirdWebView* strong_self = weak_self) {
            auto* event = Ladybird::create_context_menu_mouse_event(strong_self, position);
            [NSMenu popUpContextMenu:strong_self.link_context_menu withEvent:event forView:strong_self];
        }
    });

    self.image_context_menu = Ladybird::create_application_menu([self view].image_context_menu(), [weak_self](Gfx::IntPoint position) {
        if (LadybirdWebView* strong_self = weak_self) {
            auto* event = Ladybird::create_context_menu_mouse_event(strong_self, position);
            [NSMenu popUpContextMenu:strong_self.image_context_menu withEvent:event forView:strong_self];
        }
    });

    self.media_context_menu = Ladybird::create_application_menu([self view].media_context_menu(), [weak_self](Gfx::IntPoint position) {
        if (LadybirdWebView* strong_self = weak_self) {
            auto* event = Ladybird::create_context_menu_mouse_event(strong_self, position);
            [NSMenu popUpContextMenu:strong_self.media_context_menu withEvent:event forView:strong_self];
        }
    });
}
